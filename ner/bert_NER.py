...
class customizedProcessor(DataProcessor):
    ...
    def get_labels_info(self):
        ...
        return num_tags, tags_map, slot_class_weights
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(InputExample(guid=guid, text=text, tags=tags))
        return examples
...
def convert_single_example(ex_index, example, tags_map,
...
def file_based_convert_examples_to_features(
        examples, tags_map, max_seq_length, tokenizer, output_file):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 tag_ids, num_tags, slot_class_weights, use_one_hot_embeddings):
    ...
    return (loss, sequence_logits, slot_predict)

def model_fn_builder(bert_config, num_tags, slot_class_weights, init_checkpoint,
                     learning_rate,
    ...
    def model_fn(features, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, sequence_logits, decode_tags) = create_model(
            bert_config, is_training, input_ids, input_mask, segment_ids,
            tags_ids, num_tags, slot_class_weights, use_one_hot_embeddings)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(tags_ids, decode_tags, num_tags, input_mask):
                ...
                evl_metrics = {'slot_accuracy': slot_accuracy,
            ...
            eval_metrics = (metric_fn, [tags_ids, decode_tags, num_tags, input_mask])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
...
def main(_):
    ...
    num_tags, tags_map, slot_class_weights = processor.get_labels_info()
    ...
    if FLAGS.do_train:
        ...
        file_based_convert_examples_to_features(
            train_examples, tags_map, FLAGS.max_seq_length, tokenizer, train_file)
    ...
    if FLAGS.do_eval:
        ...
        file_based_convert_examples_to_features(
            eval_examples, tags_map, FLAGS.max_seq_length, tokenizer, eval_file)
    ...
    if FLAGS.do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, tags_map,
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            num_written_lines = 0
            output_line = "\t".join(
                ['sentence', 'slot_pred', 'slot_label', 'slot OX']) + "\n"
            ...
            for index, item in enumerate(result):
                ...
                output_line = "\t".join(
                    [tokens, pre_tags, label_tags, slot_compare]) + "\n"
                writer.write(output_line)
                num_written_lines += 1
